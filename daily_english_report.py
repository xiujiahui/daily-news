...
# 添加星期几显示
weekday_num = datetime.now().weekday()  # 0=周一, 6=周日
weekday_chinese = ['星期一', '星期二', '星期三', '星期四', '星期五', '星期六', '星期日']
weekday_display = weekday_chinese[weekday_num]
today_with_weekday = f"{today_chinese} {weekday_display}"

# 计算前一天日期（用于新闻获取）
yesterday = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')
yesterday_chinese = (datetime.now() - timedelta(days=1)).strftime('%Y年%m月%d日')
yesterday_weekday_num = (datetime.now() - timedelta(days=1)).weekday()
yesterday_weekday_display = weekday_chinese[yesterday_weekday_num]
yesterday_with_weekday = f"{yesterday_chinese} {yesterday_weekday_display}"

print(f"📅 今日日期: {today_with_weekday}")
print(f"📅 新闻日期: {yesterday_with_weekday}（前一天）")
...
def translate_weather_description(description):
    ...
    # 检查完整匹配
    for eng, chi in weather_mapping.items():
        if eng in desc_lower:
            return chi
    
    # 如果没有匹配，尝试部分匹配
    if any(word in desc_lower for word in ['sun', 'clear', 'fair']):
        return '晴朗'
    elif any(word in desc_lower for word in ['cloud']):
        return '多云'
    elif any(word in desc_lower for word in ['rain']):
        return '雨'
    elif any(word in desc_lower for word in ['snow']):
        return '雪'
    elif any(word in desc_lower for word in ['fog', 'mist', 'haze']):
        return '雾'
    elif any(word in desc_lower for word in ['wind']):
        return '有风'
    elif any(word in desc_lower for word in ['storm', 'thunder']):
        return '雷雨'
    
    # 如果无法翻译，返回原描述（假设已经是中文）
    return description
...
def get_cet6_words():
    """获取5个六级水平的英语单词，包含中文释义、例句及发音"""
    print("🔍 正在获取六级英语单词（每日五次）...")
    
    prompt = f"请提供{today_chinese}的5个英语六级水平单词，必须严格遵循以下要求：\n1. 单词必须是新的，不能与之前重复；\n2. 格式如下：\n1. 单词：[单词]\n   发音：[音标]\n   中文释义：[中文意思]\n   例句：[英文例句]\n   例句中文：[中文翻译]\n\n2. 单词：[单词]\n   发音：[音标]\n   中文释义：[中文意思]\n   例句：[英文例句]\n   例句中文：[中文翻译]\n\n3. 单词：[单词]\n   发音：[音标]\n   中文释义：[中文意思]\n   例句：[英文例句]\n   例句中文：[中文翻译]\n\n4. 单词：[单词]\n   发音：[音标]\n   中文释义：[中文意思]\n   例句：[英文例句]\n   例句中文：[中文翻译]\n\n5. 单词：[单词]\n   发音：[音标]\n   中文释义：[中文意思]\n   例句：[英文例句]\n   例句中文：[中文翻译]\n\n请确保单词是六级水平，例句地道准确，且每天提供不同的单词。"
        ...
        default_word_sets = [
            ...
            [
                ...
                },
                {
                    ...
                    'example_zh': '她务实的方法高效地解决了问题。'
                },
                {
                    'word': 'meticulous',
                    'pronunciation': '/məˈtɪkjələs/',
                    'definition': '一丝不苟的，极其仔细的',
                    'example': 'His meticulous attention to detail ensured the project\'s success.',
                    'example_zh': '他对细节的一丝不苟确保了项目的成功。'
                },
                {
                    'word': 'prolific',
                    'pronunciation': '/prəˈlɪfɪk/',
                    'definition': '多产的，丰富的',
                    'example': 'She is a prolific writer, having published over 20 novels.',
                    'example_zh': '她是位多产的作家，已经出版了20多部小说。'
                }
            ],
            # 第二组默认单词
            [
                ...
                },
                {
                    ...
                    'definition': '有弹性的，恢复力强的',
                    'example': 'Children are often more resilient than adults in adapting to change.',
                    'example_zh': '在适应变化方面，儿童通常比成人更有弹性。'
                },
                {
                    'word': 'alacrity',
                    'pronunciation': '/əˈlækrəti/',
                    'definition': '敏捷，欣然同意',
                    'example': 'She accepted the challenge with alacrity and enthusiasm.',
                    'example_zh': '她欣然接受了挑战，充满热情。'
                },
                {
                    'word': 'auspicious',
                    'pronunciation': '/ɔːˈspɪʃəs/',
                    'definition': '吉利的，有利的',
                    'example': 'The sunny morning was an auspicious start to their wedding day.',
                    'example_zh': '阳光明媚的早晨是他们婚礼日的吉兆。'
                }
            ],
            # 第三组默认单词
            [
                ...
                },
                {
                    ...
                    'example_zh': '新证据完全证明了被告的清白。'
                },
                {
                    'word': 'fastidious',
                    'pronunciation': '/fæˈstɪdiəs/',
                    'definition': '挑剔的，苛求完美的',
                    'example': 'He is fastidious about his appearance, spending hours choosing the right outfit.',
                    'example_zh': '他对自己的外表很挑剔，花几个小时挑选合适的服装。'
                },
                {
                    'word': 'gregarious',
                    'pronunciation': '/ɡrɪˈɡeəriəs/',
                    'definition': '爱交际的，群居的',
                    'example': 'She is naturally gregarious and enjoys large social gatherings.',
                    'example_zh': '她天生爱交际，喜欢大型社交聚会。'
                }
            ],
            # 第四组默认单词
            [
                ...
                },
                {
                    ...
                    'example': 'Despite being insulted, he made a magnanimous gesture of forgiveness.',
                    'example_zh': '尽管受到了侮辱，他还是做出了宽宏大量的原谅姿态。'
                },
                {
                    'word': 'obfuscate',
                    ...
                    'example_zh': '政客试图用复杂的统计数据来混淆这个问题。'
                },
                {
                    'word': 'paradigm',
                    ...
                    'example_zh': '这一发现代表了我们对物理学理解的范式转变。'
                }
            ]
        ]
    ...
    try:
        for part in response.strip().split('\n\n'):
            if part.startswith('1.') or part.startswith('2.') or part.startswith('3.') or part.startswith('4.') or part.startswith('5.'):
...
def get_hangzhou_weather():
    ...
    if weather:
        # 翻译天气描述为中文（如果返回的是英文）
        weather['description'] = translate_weather_description(weather['description'])
...
for i, word_data in enumerate(word_info, 1):
    ...
    words_html += f'''    <div class="word-section">
        ...
        <div class="pronunciation">
            {pronunciation}
            <button class="pronunciation-btn" data-word="{word}" style="margin-left: 10px; background: none; border: none; cursor: pointer; font-size: 18px;">🔊</button>
        </div>
...
html_content = html_template.format(
    today=today,
    today_chinese=today_with_weekday,
